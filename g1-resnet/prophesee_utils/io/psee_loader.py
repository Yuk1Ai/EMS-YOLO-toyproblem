...
class PSEELoader(object):
    ...
    def __init__(self, datfile):
        ...
        self._start, self.ev_type, self._ev_size, self._size = self._binary_format.parse_header(self._file)#104,0,8,[240,304]
        assert self._ev_size != 0
        if self._extension == "dat":
            self._dtype = self._binary_format.EV_TYPE#(t,u4)(_,i4)
        elif self._extension == "npy":
        ...
        for dtype in self._dtype:
            if dtype[0] == '_':
                ...
                self._decode_dtype.append(dtype)#(t,u4)(x,u2),(y,u2)(p,u1)

        # size
        self._file.seek(0, os.SEEK_END)#seek和tell是文件指针
        ...
        self.current_time = 0
        self.duration_s = self.total_time() * 1e-6#换算到毫秒
    ...
    def load_delta_t(self, delta_t):
        ...
        :param delta_t: (us) slice thickness切片厚度
        :return: events
        ...
        final_time = self.current_time + delta_t#?没懂作用
        ...
        batch = 100000
        event_buffer = []
        # data is read by buffers until enough events are read or until the end of the file没懂
    ...
    def seek_time(self, final_time, term_criterion=100000):
        ...
        它将在缓冲区中加载这些事件并进行随机搜索排序，以便结果始终准确
        """
    ...
    def total_time(self):
        """
        get total duration of video in mus, providing there is no overflow#视频总持续时间，muse单位
        :return:
        """
